...
int main (){
	...
	int contE=0, contR=0, contR_flag=0; 			//Contador contE para limitar o numero de elfos. Ambos sao usados na pthread_create
	if (pthread_create(&Santathreads[0],NULL,Santa,NULL)){
		printf ("Nao foi possivel criar o Santa");
		exit(EXIT_FAILURE);
	}
	while(1){
		//sleep(0.8);					//Para facilitar a visualizacao
		//printf("\n%i",cont);
		ticket = rand()%4;     //Sorteia o ticket
		switch(ticket){					//Cria as treads aleatoriamente com uma chance em quatro de criar reindeer
			case 0:
				if(contR < REINDEER && contR_flag == 0){		//impedindo assim que se tenha mais de 9 reindeers
	...
	if(count_reindeer>9 || count_elf>3){
